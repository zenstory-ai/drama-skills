...
**实测口径**：模型响应的是 `控制` 这半句的肯定表述——正文明确写「保留起始构图、站位、机位高度」时，
构图会被准确带过来；只写 `fully_preserved` 或只写「不得控制构图」时，构图都不会被带过来。
所以**需要保住构图的槽位（典型是 `用途：起始帧`）必须在 `控制` 里点名构图**，漏写就拿不到这个锚点，
在别处补多少 `不得控制` 都无效。`不得控制` 照常写，但它是给 owner 与 reviewer 看的责任边界，
不是对模型的开关。
...
实测口径：H3 中文自然语速约 **4.1 字/秒**，容量约 `4.1 × 镜长`；超出后先赶语速（最快约 2.5 倍），
再超过约 9 字/秒就**静默截断**，接口不报错。别把"没报错"当成台词说完了。
