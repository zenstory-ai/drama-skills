...
上面三种来源里，「声音库里的一条」是唯一不需要向任何人取得授权的：它不复现某个具体的人。
...
音色库的名字描述的是人设，不是音区。本套件实测过三个名字都指向「低沉成熟男声」
的预置音色，基频全部落在 93–94 Hz——彼此相差 1 Hz，放进同一场戏里就是同一个人。
**靠读名字分别挑，挑出来的大概率是同一个声音**，而这件事在文档上完全看不出来。

所以「对比着选」在预置音色上是一个可执行的程序：

1. 把同一批候选**用同一句台词**各合成一条——变量只能是音色；
2. 量基频中位数与谱包络；
3. 要求任意两个角色的间距**大于各自组内的极差**；
4. 不满足就换一个候选再量，不要靠调语速或音调去凑。

第 3 步用相对判据而不是固定 Hz 数，因为不同音色自身的离散度不一样：
同样念三句话，有的音色组内极差 8 Hz，有的 18 Hz。固定阈值会同时放过一对
真混的和拦下一对真分得开的。

第 4 步那句限制有理由：语速与音调是**表演**，按本文档开头的界线它们不属于身份。
靠拉开语速制造的区分，在角色情绪变化时就消失了。
...
以及作为配音本的实际人声。两个去处用同一条参考，是同一个角色在整部片子里只有一个声音的
前提——每镜各自生成人声时，没有任何东西约束它们是不是同一个人。
