...
1. 先确认每个场景动作、对白、声音、画面文字和转折由哪些镜头承载，再用已接受的目标模型原生时长与交付路线检验生成可行性。
...
- 已声明原生时长时，每个一镜一生成的素材、或多镜打包的整个容器，都落在其最短与最长秒数内；
  容器成员不各自套用单次生成下限。先定叙事所需时间，短动作可从合法素材中选取入剪区间，或走已支持的
  多镜容器；不为了填满生成下限添加无职责停顿。生成素材剩余时间仍需有明确状态，成片不必照单全收。
  长于上限的动作按可见状态闭合点拆镜，每镜仍各有唯一职责与可信交界。
- creator-first 的「时长」保留素材/生成时长含义；需要选取片段时，在 SHOT 与对应 MOTION 的正文外
  同写可选 `- 入剪区间：0.5-2s`。未写沿用全段；实际素材到位后核对本镜承载义务与衔接，保留已接受的省略。
  使用普通十进制秒数，满足 `0 ≤ 起点 < 终点 ≤ 时长`，两边区间与素材时长数值一致；详见
  [阶段契约](references/stage-contract.md)。
