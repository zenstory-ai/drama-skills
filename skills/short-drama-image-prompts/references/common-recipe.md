...
上一节只说了空泛的审美词没有信息量，没说该拿什么填。填的是两样具体东西。

**一是光学口径。** 焦段、机位高度、光位这类摄影参数比形容词更能落地：写死焦段，模型要算出
对应的透视压缩与焦外衰减；只写「电影感」，得到的是一层平的滤镜。这不是要求每张图都摆满参数，
而是说人像与特写这类靠脸吃饭的画面，镜头口径要写出来。

**二是表面的真实状态。** 皮肤的毛孔与纹理、布料的磨损与褶皱、金属的指纹与氧化——这些要**正面
点名**，不能指望模型自己带上。默认不点名时，模型倾向输出打磨过的表面：皮肤没有孔隙、布料没有
穿过的痕迹。同理，暗示影棚打磨与修饰的词（精致、无瑕、完美）会把画面推向同一个方向。

- **`craft_default`**：人像与特写点名镜头口径与表面状态；其余画面按需要写。
- **`craft_default`**：不写暗示打磨与修饰的词；要「好看」时落到具体的光位与材质选择。
- **`taste_option`**：写实程度仍由视觉风格决定——有的项目就是要打磨过的表面，这时上面两条
  被风格覆盖，而不是被忘记。

**写「没有 X」之前先确认 X 在真实世界里本来存不存在。** 本套件的角色板写过
「面料平整、没有褶皱与磨损」——那句想表达的是「制服干净」这个造型事实，
却顺手禁掉了穿着必然形成的折痕。造型事实要保留（干净、没有泥、没有破损），
反写实的那半句去掉。负面约束最容易在这里越界：它针对的应该是可预见的失败，
不是真实世界的常态。

**这类改动用盲看验证，不要用纹理统计量做闸门。** 实测在同一批图上试过两个
像素指标：高频能量均值落在组内波动里测不出，局部纹理变异系数勉强可分但
方向与直觉相反（铺满毛孔反而让纹理分布更均匀）。而盲评一次就把两组精确分开。
