...
| IMG-08 | reviewed_invariant | A claim about reference pixels requires a creator/reference-owner description or authorized input-reference observation bound to the inspected bytes and version; when inspected, compare identity, current story state, visible information and screen/object geometry with the intended use. Otherwise admission stays unresolved, and prompt text cannot stand in for pixel evidence. |
| IMG-09 | reviewed_invariant | Each reference states its purpose, what may be copied, and what must not be copied; a reference cannot redefine facts outside its declared control scope. Physical shape and spatial relationships are distinct from view-dependent projection; boundary-frame references obey the visibility and state of their declared moment, while other roles need not match every target-frame attribute. |
