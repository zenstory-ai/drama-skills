...
当前已确认 job 是本轮唯一生产工作单元；运行结束后回报结果并交还控制权，不自动准备下一批，也不因
普通生成请求扩大成整集审查。若用户本轮已明确要求交付样片/全集并复核质量，可把现有结果交给
`$short-drama-review` 做只读审查；这不授权重新生产或付费重试。
...
成功后回报实际输出路径、媒体类型和运行状态；不要把“adapter 返回成功”写成创作验收或质量结论。
...
如需质量复核，报告可把已有结果交给 `$short-drama-review`。普通生成请求不自动启动整集复核；用户已
要求审查本次样片、生产结果或全集时，复核现有媒体属于已授权的只读工作，可继续执行。复核结论不授权
自动重投、补拍或付费重试。
