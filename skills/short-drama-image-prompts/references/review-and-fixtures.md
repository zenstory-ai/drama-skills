...
| 参考证据 | 实际画面是否符合用途及当下状态？ | 已检查文件/版本、剧情时点、状态与几何对照；不可见则未验证 |
...
参考媒体可访问时，检查实际文件并记录版本、用途与覆盖范围；按控制范围核对身份、形制、空间关系、
状态和可见信息。边界帧按其声明时点核对；其他用途不要求所有属性匹配目标，正常机位变化也不等于
几何漂移。只有引用已接受约束与具体冲突，才判 `REVISE`，范围外内容记录误带风险与处置。媒体不可见
时保持 `unverified`；提示词与文件名不能充当像素观察，也不能据此宣称未发现画面问题。
