...
| EDT-18 | craft_default | Grain is one value for the whole delivery, declared in the cut list's delivery spec and applied after assembly. Applied per cut it becomes one more difference between segments, which is the defect it exists to cover; the texture also has to change frame to frame, because static noise reads as dirt on the lens rather than as film. It covers surface cleanliness only — it cannot cover identity drift, a wrong key direction, or a failed action, and using it that way hides a defect instead of fixing it. |
