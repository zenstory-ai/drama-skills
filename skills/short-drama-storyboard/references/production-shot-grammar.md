...
| 子镜密度 | 按新信息、行动后果和观众注意决定何时切换，不设通用秒数。连续几镜若只把同一件事做完而没有新增观看价值，先压缩重复；静止构图也可让等待、微小变化或声音推动压力，不为避免“静态”硬加运镜。 |
| 单元时长 | 先从必须保留的动作、反应、对白与最终状态反推观看时间，再选择合法素材长度或已支持的多镜打包路线。一次生成多长与最终选用多久分开记录；短动作不自动扩成全段停顿。冻结关键帧只描述一个瞬间，任何长度的镜头都需要运动状态链，长镜是否拆分由内容与可执行性决定。 |
...
creator-first 在《分镜.md》末尾分别核对素材总秒数与预计入剪总秒数：声明「入剪区间」的镜头贡献
终点减起点，未声明的贡献「时长」；这是原速顺接的预计总秒数，变速或重叠转场另在现有说明核算，
再与目标比较，未定值显式保留。它是剪辑计划，
不能证明尚未观察的素材已可用。压缩时核对本镜职责，不能漏掉它负责承载的必要信息；
已接受的省略或其他镜头承载安排继续有效，重复本身不构成删减理由。

以下保留结构化管线的既有口径，不把 `duration_seconds` 静默改成入剪时长：
