...
上面那几条是机位和机位打架。还有一类更常见、更不容易看出来：**机位要求与主体动作要求
互相抵消**。「他快步冲出门」配「镜头极缓推进」不是矛盾修辞，是两条速度指令——模型往往
只执行一条，而它选哪条不由你决定。

修法不是删掉其中一条，是**把两者用一个关系动词连起来**，让它们变成一件事而不是两件：

```text
差：他快步冲向门口。镜头缓慢推进。
好：镜头跟随他冲向门口，保持他在画面右三分之一，门框始终在画面内。
```

关系动词（跟随、匹配速度、被他甩在后面、始终保持某个构图关系）把"谁带着谁动"说清楚。
说不清楚时，让机位不动——**一个能读的固定镜头强过一次方向不明的运动**。

### 一次运动只写四件事

实践里有效的写法是：**一个动作、一个方向、一个速度、一个必须保持不变的东西**。
最后那一项最容易漏，而它是唯一能让人判断这次运动做没做对的东西——「推进到她的头肩，
**轴线不变、门框始终在画面左缘**」里，后半句才是可验收的。

常见的无效写法：把几个运动动词堆在一起；只说动不说往哪动；不给速度或时长；
把推轨和变焦混为一谈（两者的透视变化完全不同）；以及「更有张力一些」这类没有
画面所指的词。

### 运镜术语属于方言

推轨、变焦、摇、甩、升降这些词在不同目标模型里的解析并不一致——有的模型的官方指南
推荐 `push in` 而完全不收录 `dolly in`。**不要把一个模型上验证过的运镜词表照搬到另一个
模型的正文里**；术语与写法以目标模型的方言文档为准，方言里没有记载的词按未验证对待。
