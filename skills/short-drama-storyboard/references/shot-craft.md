...
一次打击、否定、揭穿或翻盘，画面上有两个人：**说出它的人，和挨了它的人**。
只拍前者，观众收到的是一句评价；拍了后者，观众才收到一次事件。

这不是「多给一个反应镜头会更好」的锦上添花。剧情上被推翻的那个人如果全程只有后脑、
只在群像里占几十个像素，观众其实不知道该看谁输——那句最狠的台词就只是一个人在说话，
爽点不成立。**分镜阶段就要决定谁承受这一下、哪一镜给他**，不要留给剪辑去补，
因为剪辑只能在已经生成的素材里选，选不出一个没拍过的反应。

具体到写法：

- 台词落点是针对某个在场的人时，检查分镜里有没有一镜的画面主体是**那个人**；
- 没有就补一镜，通常很短（一到两秒足够：停手、抬眼、想开口又闭嘴）；
- 承受者不在场时，那一下的后果要落到别的可见东西上（屏幕、门、被推开的文件），
  否则这次打击在画面上没有发生过；
- 群像镜头**不算**承受者反应：一张在四个人里的脸承担不了「他输了」这件事。

**这条只在剧情确实存在承受者时生效。** 不为每句台词配反应镜头，也不把它变成正反打配额——
安静的场次、独角戏、纯信息交代不适用。
...
分镜每镜都已经写了「终点」。那一句是文字，没有画面；生成时模型按自己的理解收尾，
而镜头的职责常常恰恰落在收尾那一下——谁最后没有抬头、文件最后是不是还合着、
灯最后亮没亮。只锚起点时，这些全部是模型自由发挥的余地。

`### 收尾关键帧提示词` 把那一句渲染成画面的另一端。研究侧的结论是成对锚定首尾优于只给
稀疏关键帧，本套件也自己测过一次，差距比预期大：

同一镜、同一提示词、同一起始帧，唯一变量是给不给尾帧，每臂 3 次。只给起始帧时，
**三次的末帧都离起点比离终点更近**——动作发生了一点，但没有走到文档写的那个状态；
绑了尾帧的三次都落在终点上。方差的变化比均值更明显：从 ±0.016 降到 ±0.0002，
终点这件事从抽卡变成了确定的。

也就是说，**「终点」只写成散文时，它很可能不会发生**。这一条和「散文秒数不构成时间
控制」是同一类事实：文档里写下来的状态，不等于模型会把画面带到那里。

顺带排除了一个相邻的担心：绑尾帧不会把画面定住。总运动量是只给起始帧的 0.86 倍，
全程都在动、只在最后收住。

**不是每镜都要写。** 判据是一句话：**终点和起点在画面上是不是同一格**。

| 写 | 不写 |
|---|---|
| 终点有起点没有的可见事实（灯亮了、文件摊开了、人转过头了） | 起止姿态一致，只是时间过去了 |
| 本镜职责就是那个变化本身 | 变化在画外发生、画面上看不出来 |
| 承受者反应镜——那一下落在脸上的结果就是终点 | 纯运镜镜头，主体状态不变 |

写的时候规则与起始帧完全对称：**只投影终点**，删掉起点才有、终点已经不在的东西。
两头都写全的关键帧不是"更完整"，是把一格画面画成了两格，模型会在中间摇摆。

绑定与目标模型的支持情况见对应方言文档——不是每个模型都收尾帧，收的也未必叫同一个名字。

## 一镜到底还是切开

生成管线里这个问题和实拍不一样，因为**切口的代价在两种媒介里是反的**。

实拍时切一刀几乎免费：机器停了再开，演员重来，接不上就再拍一条。生成时切一刀是
**跨镜一致性最容易崩的地方**——人物、光、色温都在这里对不上，本套件的实跑就量到过
七镜亮度散在 97–131。所以一镜到底在生成里买到了一样实拍不需要买的东西：**没有接缝**。

但它同时卖掉了两样，而这两样在生成里比在实拍里更贵。

**卖掉的第一样：选择。** 实拍可以拍二十条，导演还能指导演员改。生成不能指导，
只能重摇骰子——**每一条都是抽卡**。切开时，一句话演砸了只重出那一镜、几秒钟的钱；
一镜到底时，十五秒里有一秒不对，整条作废、整条重付。片段越长，要同时成立的事越多。

**卖掉的第二样：节奏。** 这条是实测出来的，而且比预期严重。分镜生成时每一镜的时长
是一份契约，「这句话在第几秒说」由镜头边界强制；改成一镜到底之后，四句台词各自
什么时候发生**完全由模型决定**——三次里有两次把全部台词挤进了前三分之一，剩下十秒
无事可做。这不是波动，是控制手段没了。
...
- **内容就是那段不间断的时间**——沉默、僵持、等待。切开之后它变成几个反应镜头，
  是另一种效果，不一定更差，但确实是另一件事。
- **空间关系就是内容**——谁在这个房间里、谁离谁多远、谁背对着谁。切开就要靠观众
  自己拼，而生成的空间本来就不可靠。
- **而且你愿意整条收下**。这一条是硬门槛：不打算挑就别切，打算挑就别用长镜。

**用切开：**

- **落点在切上**。包袱、打脸、翻转——这类节拍的力量常常来自那一刀本身，
  用运镜代替它通常更弱。
- **有承受者要给**（`SHT-27`）。一镜到底只能靠调度把两个人放进同一格；
  做不到时，反应镜头是唯一的办法。
- **表演需要挑**。生成的表演是抽卡，切开才能只留住成立的那几秒。
- **节奏必须精确**。每句话落在第几秒由你决定时，只能靠镜头边界去锁。

### 反过来也成立：切开可以造出连续

一个和直觉相反、但实践里反复出现的做法：**同一个情绪的崩塌，用几条各自表情不同的
短镜快切，往往比一条不间断的表演更像一次完整的崩溃**。

原因回到上面那条「选择」：生成的每一条都是抽卡，一条十五秒的表演里总有几秒是平的；
而三条两秒的素材里，可以只留住那三个真正到位的瞬间，让观众的脑子把中间补上。
剪辑在这里不是在切碎连续性，是在**替观众挑选**——这正是长镜头交不出来的东西。

所以「表演要连续」不等于「必须一镜到底」。先问这段连续性是靠**画面不断**建立的，
还是靠**观众把几个瞬间连起来**建立的；后者更常见，而且在生成管线里成本低得多。

### 一条容易搞混的

**多镜容器不是一镜到底，两者相反。** 容器里仍然有切口，只是切口由时间戳而不是剪辑
决定——它同时失去了剪辑的选择权和长镜头的无接缝，实测也确认它不改善跨镜一致性。
要一镜到底就写成一个镜头，不要写时间戳分镜。
