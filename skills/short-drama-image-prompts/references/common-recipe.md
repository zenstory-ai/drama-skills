...
输入参考的准入检查和输出意图必须使用同一组事实。只有创作者/参考图权利人给出可以核对的画面说明时，把状态写为
`creator_described`；运行环境允许检查输入参考并实际看过指定文件/版本时，才写
`visually_inspected`。只检查提示词正文不算看过画面。只有文本记录、媒体不可见或未获授权时保持
`unverified`，不能编造观察结论。

实际检查围绕这张参考声明的用途进行：

- 核对身份锚点和允许控制的造型/状态，没有把临时状态固化为身份；
- 区分实体形制、空间地理与机位造成的画面投影差异：按用途核对前两者，不把换视角后的投影差异当作结构漂移；
- 核对可辨文字、近似字形、号码、标牌、界面、水印和字幕残留；
- 起始/结束帧参考逐项对照其声明边界允许出现的信息与状态；不能带入未授权的事实。
  时间边界按当前镜头而非故事年代判断，剧本或创作者已接受的非线性表达照常保留。
  身份、地理或风格参考只在各自控制范围内匹配，不要求其姿态、衣着或画面投影全部等同于目标帧；
  范围外内容仍需评估误带风险，不能仅凭“不得控制”就宣称风险消失。

检查结论写明文件/版本、用途和覆盖范围；抽查一张图不能证明其他版本或整段媒体正确。发现冲突时，
修正参考边界、换用正确状态的参考或退回负责方，不能靠在正文里追加相反描述宣称风险已消失。
...
- 这一步只记录输入参考图的检查资料与可见风险；本提示词阶段不生成或验收成片，也不从
  提示词声称文字已经正确渲染。
...
- 边界帧泄露该时刻不允许出现的信息，或参考在其声明控制范围内违背已接受的形制、空间关系或状态；
...
1. **结构**：检查 ID、版本、类型必需字段、来源引用、参考图准入状态、来源政策到呈现方法
   的对应关系，以及可以直接确定的矛盾。
2. **内容**：审查者引用资产字段、提示词句子和实际看过的参考文件/版本，判断身份、当前状态、
   信息权限、场景地理和尺度是否保持；媒体不可见时明确未验证，不能只说“不够高级”。
