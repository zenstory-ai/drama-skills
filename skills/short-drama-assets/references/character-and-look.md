...
上表右列那些锚点——细微的五官不对称、浅色小痣、依赖真实皮肤纹理的痕迹——**作为跨镜身份锚点
确实不可靠**，那一列没有写错：剪影通道里它们根本不出现，指望它们认人会落空。

但它们还有另一个用处，而且正好是当前这类生成最缺的。**同质化的生成面孔是被「挑不出毛病」定义
的**：五官匀称、皮肤均匀、左右对称，于是一张脸和另一张脸之间没有可抓的东西。让一张脸像一个
具体的人，靠的恰恰是这些不对称与不均匀。

两件事分工清楚，不要互相顶替：

- **身份靠可复现的通道扛**——轮廓、剪影、比例、固定配饰。跨镜要认得出，只能靠这些。
- **真实感靠不可复现的细节扛**——不对称、痣、疤、牙齿排列、皮肤纹理、法令纹。它们不必每镜
  一致，也不该被要求每镜一致。

- **`craft_default`**：角色板在最小稳定识别集之外，另写一组这类细节。它们不进识别集，
  也不参与连续性核对——**要求它们跨镜一致会同时毁掉两件事**：识别集被不可靠的锚点稀释，
  真实感被「必须一致」压回到匀称。
- **`taste_option`**：题材要求偶像化外形时，这一组可以整体不写；这是创作决定，写进视觉方向。
