...
沿用 creator-first 骨架：`## MOTION-... · 中文名`，字段名依次使用「分镜、时长、入剪区间（可选）、生成方式、
输入参考图、静态视觉锚点、起始帧、状态链、终点」，正文标题使用 `### 可复制提示词`。静态视觉锚点
跟随本镜已解析的视频提示词语言并原样进入正文；正文每个非空行都以 `>` 组成 Markdown 引用块。
分镜声明了「入剪区间」时才在「时长」之后同写该字段，它留在元信息中，不进入正文。
...
5. 核对本次生成的素材时长落在目标模型档案的原生区间内：一镜一生成核对该镜，多镜打包核对容器
   总时长，不能把单次下限套在每个成员上。不合法就回分镜修订，不能留给 adapter 或暗改正文时长。
   分镜有「入剪区间」时原样抄到对应 MOTION 的元信息里，不放进可复制提示词或供应商时长参数；
   「时长」仍覆盖完整素材。完整动作与声音时间线照常写，选段核对本镜承载义务与衔接，不撤销
   已接受的省略或其他镜头承载安排；实际可剪性待素材复核，不能靠区间数字承诺。
...
- 同轨模型下的无对白镜同样要写满声音段：正面写出底声的构成与覆盖区间，把「不说话」写成在场人物的可见状态，完整性声明只列本镜实际有的层，不枚举 dialogue。非画内配乐层显式写空（H3 为 `non_diegetic_music: N/A`）——不写配乐层实测会被模型补上，语音层未观察到同样现象。
