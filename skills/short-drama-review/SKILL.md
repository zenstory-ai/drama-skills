...
- 五文档的可见 ID、引用和时长是否一致；
- 范围包含 `production_outputs` 或 `full_episode` 且媒体可读时，实际查看当前结果及其绑定参考：抽查
  中间帧、镜头边界和状态转变；只有实际听过的声轨才判断对白、口型、音效或混音。记录文件、版本、
  时间点、方法和覆盖限制；无法观察的维度标为 `PROVISIONAL`，已由当前文字直接证明的冲突仍判
  `REVISE`；
- 剧本对白、动作、声音、画面文字是否都有画面/声音承载；
- 人物、地点、道具状态和镜头起止是否连续；
- 提示词是否从准确起点到准确终点；
- 目标模型画音同轨时，可复制正文的声音段是否写成正面闭合事件集：无对白镜不靠 `No dialogue`、
  `无对白` 关闭语音通道，完整性声明也不枚举本镜没有的层（`VID-22`、`VID-23`，见
  [视觉运动审查表](references/rubric-visual-motion.md)）；
...
按 owner 分组修订要求。本轮完成后交还控制权；修改和重新生产只有用户明确请求时开始。用户已经要求
审查样片、生产结果或全集时，读取现有媒体、抽帧和复核当前版本属于本轮只读审查，无需另等一次确认。
...
- 无法读取媒体或声轨时明确限制，把依赖该维度的结论保留为 `PROVISIONAL`；不从文字或 adapter 状态
  推断脸部一致、表演、口型、混音或市场表现。
- 生产观察绑定结果版本、观察方法和范围；输入、提示词、参考或配置缺失时标未知，限制因果归因，不能泛化成通用规则。
